 ...
 * @description  第五个统计的servlet 传输数据的<br>
 * 被API使用的Tag数量、平均使用次数、使用次数分布、使用最多的前30个Tag
 */
@SuppressWarnings("serial")
public class Statistics5Servlet extends HttpServlet {

	public void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		//获取被API使用的Tag数量
		int tagCountUsedByApi = TagSimilarityAnalyze.getTagCountUsedByApi();
		//获取被API使用的Tag平均使用次数
		double averageUseofAPI = TagSimilarityAnalyze.getTagUsingAverageByApi();
		...
		for(WebTag t:listTag){
			int count = t.getTag_apis().split(",").length;
			if("".equals(t.getTag_apis())){
				zero++;
			}
			if(count == 1 && !"".equals(t.getTag_apis())){
				one++;
			}
			if(count >= 2 && count <= 5){
				twoFive++;
		...
		/***********被api使用最多的前30个tag*************/
		List<WebTag> listTagUsedByApi = TagSimilarityAnalyze.getFirstNTagsByApi(30);
		String xAxisData = "";//x轴的数据
		String seriesData = "";//要显示的数据
		for(WebTag t:listTagUsedByApi){
			xAxisData = xAxisData + "\""+t.getTag_name()+"\",";
			seriesData = seriesData + t.getTag_apis().split(",").length+",";
		}
		...
		request.setAttribute("tagCountUsedByApi", tagCountUsedByApi);
		request.setAttribute("averageUseofAPI", averageUseofAPIresult);
		request.getRequestDispatcher("statistics5.jsp").forward(request, response);
